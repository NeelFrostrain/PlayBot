                ...
                try {
                    ...
                    if (queue.textChannel) {
                        const embed = {
                            ...
                            color: 0x3498db,
                            fields: [
                                { name: 'Position in Queue', value: `#${queue.songs.indexOf(nextSong) + 1}`, inline: true },
                                { name: 'Requested by', value: nextSong.requestedBy.username, inline: true }
                            ],
                    ...
                    const downloadResult = await this.ytdlp.preDownloadAudio(nextSong.url);
                    ...
                    if (queue.textChannel) {
                        const embed = {
                            ...
                            color: 0x00ff00,
                            fields: [
                                { name: 'Status', value: downloadResult.cached ? '💾 Was cached' : '📥 Downloaded', inline: true },
                                { name: 'Ready to play', value: 'Instantly when reached in queue', inline: true }
                            ],
                    ...
                    console.error(`Pre-download failed for ${nextSong.title}:`, error.message);
                    ...
                    if (queue.textChannel) {
                        const embed = {
                            ...
                            fields: [
                                ...
                                { name: 'Will try again', value: 'When song is about to play', inline: true }
                            ]
                        };
