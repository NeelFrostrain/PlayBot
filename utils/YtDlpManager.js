...
class YtDlpManager {
  ...
  async downloadAudio(url, progressCallback) {
    ...
    return new Promise((resolve, reject) => {
      const args = [
        ...
        "128K",
        "--output",
        path.join(this.downloadDir, "%(id)s.%(ext)s"),
        "--no-playlist",
        "--user-agent",
        "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
        "--referer",
  ...
  createProgressEmbed(progress, videoInfo, additionalInfo = {}) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(
        progress < 50 ? "#ff9900" : progress < 90 ? "#ffcc00" : "#00ff00"
      )
      .addFields(
        {
        ...
        {
          name: "Duration",
          value: this.formatDuration(videoInfo.duration),
          inline: true,
        },
        {
          name: "Uploader",
          value: videoInfo.uploader || "Unknown",
          inline: true,
        }
      );
